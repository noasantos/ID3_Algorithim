...
def runID3_ALLDATA(datasource):
    ...
    buildTreeImg(rootNode, "connect4")

def runID3_CONNECT4():
    datasource = 'datasets/connect4.csv'
    df = preProcess(pd.read_csv(datasource, keep_default_na=False))
    rootNode = buildTree(df)
    buildTreeImg(rootNode, "connect4")


def buildTreeImg(rootNode, dataset_name, type="TEST", output_folder=r'C:\Users\noaca\Desktop\ID3_Algorithim\DecisionTreeGraphs'):
...
#runID3_ALLDATA('datasets/weather.csv')
#runID3_TRAINTEST('datasets/restaurant.csv')
